...
export const createPost = async (req, res) => {
    try {
        ...
        if ( !text && !img) {
            return res.status(400).json({ error: "Please enter a text or an image" })
        }
...
export const deletePost = async (req, res) => {
    try {
        const post = await Post.findById(req.params.id); // Find Post instead of User
        ...
        if (post.user.toString() !== req.user._id.toString()) { 
            return res.status(403).json({ error: "You are not authorized to delete this post" }); // Use 403 Forbidden for authorization errors
        }
...
export const getLikedPosts = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json(likedPosts); // Return the populated likedPosts
    } catch (error) {
